...
using WebsiteBH.Models.Payments;
using System;
...
namespace WebsiteBH.Controllers
{
    public class ShoppingCartController : Controller
    {
            ...
            if (ModelState.IsValid)
            {
                ...
                if (cart != null)
                {
                    ...
                    foreach (var sp in cart.Items)
                    {
                        ...
                        strSanPham += "<td>" + WebsiteBH.Common.Common.FormatNumber(sp.TotalPrice, 0) + "</td>";
                        strSanPham += "</tr>";
                    ...
                    contentCustomer = contentCustomer.Replace("{{ThanhTien}}", WebsiteBH.Common.Common.FormatNumber(thanhtien, 0));
                    contentCustomer = contentCustomer.Replace("{{TongTien}}", WebsiteBH.Common.Common.FormatNumber(TongTien, 0));
                    WebsiteBH.Common.Common.SendMail("ShopOnline", "Đơn hàng #" + order.Code, contentCustomer.ToString(), req.Email);
                    ...
                    contentAdmin = contentAdmin.Replace("{{ThanhTien}}", WebsiteBH.Common.Common.FormatNumber(thanhtien, 0));
                    contentAdmin = contentAdmin.Replace("{{TongTien}}", WebsiteBH.Common.Common.FormatNumber(TongTien, 0));
                    WebsiteBH.Common.Common.SendMail("ShopOnline", "Đơn hàng mới #" + order.Code, contentAdmin.ToString(), ConfigurationManager.AppSettings["EmailAdmin"]);
                    cart.ClearCart();
        ...
        public ActionResult AddToCart(int id, int quantity)
        {
            ...
            {
                return Json(new { Success = false, msg = "Sản phẩm không tồn tại", code = -1, Count = 0 });
            }
            ...
            item.Price = checkProduct.IsSale ? (decimal)checkProduct.PriceSale : checkProduct.Price;
